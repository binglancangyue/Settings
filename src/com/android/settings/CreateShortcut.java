...
import com.android.settings.Settings.HotspotSettingsActivity;
...
/* SPRD added for Bug757200, change icon for some shortcut @{*/
...
public class CreateShortcut extends LauncherActivity {
    ...
    protected List<ResolveInfo> onQueryPackageManager(Intent queryIntent) {
        ...
        for (int i = activities.size() - 1; i >= 0; i--) {
            ...
            /*SPRD added for bug 692494,hide the datausage in cmcc version @{*/
            if (info.activityInfo.name.endsWith(Settings.DataUsageSummaryActivity.class.getSimpleName()) && DisableDataUsage.getInstance().isCmcc()) {
                activities.remove(i);
            }/* @}*/
            /*SPRD 715674 hide the shortcut of audioprofile and memoery @{*/
            if (info.activityInfo.name.endsWith("AudioProfileSettings")) {
                if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                    activities.remove(i);
                }
            }
            //SPRD 750135 cts case Cross profile intent filters are set fail
            if (info.activityInfo.name.endsWith(Settings.SoundSettingsActivity.class.getSimpleName())) {
                activities.remove(i);
            }
            //SPRD 718379  hide the shortcut of memory
            if (info.activityInfo.name.endsWith(Settings.MemorySettingsActivity.class.getSimpleName())){
                if (UserHandle.myUserId() != UserHandle.USER_OWNER) {
                    activities.remove(i);
                }
            }
            ...
            //SPRD 794327  hide the shortcut of DataPlanUsageSummaryActivity
            if (info.activityInfo.name.endsWith(Settings.DataPlanUsageSummaryActivity.class.getSimpleName()) &&
                    !FeatureFactory.getFactory(this).getDataPlanFeatureProvider().isEnabled()) {
                activities.remove(i);
            }
            if (info.activityInfo.name.endsWith(Settings.DataUsageSummaryActivity.class.getSimpleName()) &&
                    FeatureFactory.getFactory(this).getDataPlanFeatureProvider().isEnabled()) {
                activities.remove(i);
            }
            /* @}*/
            if (info.activityInfo.name.endsWith(Settings.LocationSettingsActivity.class.getSimpleName())) {
