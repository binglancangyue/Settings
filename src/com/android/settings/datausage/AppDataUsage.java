...
public class AppDataUsage extends DataUsageBase implements Preference.OnPreferenceChangeListener,
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (mAppItem.key > 0) {
            if (mPackages.size() != 0) {
                try {
                    String packageName = (args != null) ? args.getString(AppInfoBase.ARG_PACKAGE_NAME, "") : "";
                    if (TextUtils.isEmpty(packageName)) {
                        packageName = mPackages.valueAt(0);
                    }
                    ApplicationInfo info = mPackageManagerWrapper.getApplicationInfoAsUser(
                            packageName, 0, UserHandle.getUserId(mAppItem.key));
