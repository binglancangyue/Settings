...
public class CellDataPreference extends CustomDialogPreference implements TemplatePreference {
    ...
    public void setTemplate(NetworkTemplate template, int subId, NetworkServices services) {
        ...
        /* SPRD: Bug 708586 only check default data sub @{ */
        ...
        Log.d(TAG, "setTemplate(" + enabled + "," + mSubId + "," + dds + ")");
        /* @} */
    ...
    protected void performClick(View view) {
        ...
        /* SPRD: add for bug708331 && 894273 @{ */
        if ((mSubId != SubscriptionManager.INVALID_SUBSCRIPTION_ID)
                && mTelephonyManager.getSimState() == SIM_STATE_READY
                && WirelessUtils.isAirplaneModeOn(getContext())) {
            Toast.makeText(getContext(), R.string.toggle_data_error_airplane, Toast.LENGTH_SHORT)
            .show();
            return;
        }
        /* @} */
        if (mChecked) {
            ...
            /* SPRD: bug 894021 vice sim's mobile data option should not been opened @{ */
            if ((nextSir != null && currentSir != null &&
                    currentSir.getSubscriptionId() != nextSir.getSubscriptionId())) {
                boolean isMobileDataEnabled = mTelephonyManager.getDataEnabled();
    ...
    private void setChecked(boolean checked) {
        // SPRD:Bug 788821 mobile data switch status is not same with status bar
        Log.d(TAG, "setChecked() mChecked = " + mChecked + " checked = " + checked);
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        // SPRD:Bug 788821 mobile data switch status is not same with status bar
        Log.d(TAG, "onBindView() mChecked = " + mChecked);
