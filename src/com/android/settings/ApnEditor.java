...
public class ApnEditor extends SettingsPreferenceFragment
    ...
    private static final String[] sProjection = new String[] {
            ...
            // SPRD: add for bug733602
            ApnEditorUtils.APN_EDITABLE
    };
    ...
    public void onCreate(Bundle icicle) {
        ...
        // SPRD: FEATURE_APN_EDITOR_PLUGIN
        mAllowEmptyApn = ApnEditorUtils.getInstance(getContext()).allowEmptyApn();
        ...
        /* SPRD: FEATURE_ADD_APN_EDITABLE_FIELD @{ */
        if (icicle != null) {
            mIsEditable = icicle.getBoolean(ApnEditorUtils.KEY_IS_EDITABLE, true);
            Log.d(TAG, "[onCreate] mIsEditable: " + mIsEditable);
        }
        /* @} */

        /* SPRD added for Bug785680, intentfuzzer test failed @{ */
        if (action == null) {
            finish();
            return;
        }
        ...
        /* SPRD: Bug 597998 Apn settings crash @{ */
        if (mCursor.getCount() == 0) {
            Log.e(TAG, "No record found for uri: " + mUri);
            finish();
            return;
        }
        /* @} */
        ...
        /* SPRD: bug 897601 @{ */
        boolean isApnEditableForOperator = getContext().getResources().getBoolean(
                R.bool.is_apn_editable_with_version);
        Log.d(TAG, "onCreate: EDITED " + mCursor.getInt(EDITED_INDEX) +
                " isApnEditableForOperator = " + isApnEditableForOperator);
        // if it's not a USER_EDITED apn, check if it's read-only
        if (mCursor.getInt(EDITED_INDEX) != Telephony.Carriers.USER_EDITED &&
                ((!isApnEditableForOperator && mCursor.getInt(USER_EDITABLE_INDEX) == 0) ||
    ...
    public void onResume() {
        ...
        /* SPRD: FEATURE_APN_NOT_EDITABLE @{ */
        boolean editable = mIsEditable &&
                ApnEditorUtils.getInstance(getContext()).isApnEditable();
        /* SPRD: FEATURE_ADD_APN_EDITABLE_FIELD @{ */
        if (!mNewApn && !editable) {
    ...
    private void fillUi() {
        if (mFirstTime) {
            ...
            /* SPRD: add for bug733602 @{ */
            String apnType = mCursor.getString(TYPE_INDEX);
            /* SPRD: Bug Bug 757343 add for operator which APN could not show XCAP  @{ */
            final boolean allowShowXCAP = ApnEditorUtils.getInstance(getContext())
                    .isApnAllowDisplayXCAP(mCursor);
            final boolean editable = ApnEditorUtils.getInstance(getContext()).getEditable(mCursor);
            if (!allowShowXCAP && !TextUtils.isEmpty(apnType) && apnType.contains(",xcap")) {
    ...
    private String mvnoDescription(String newValue) {
        ...
        if (mvnoIndex == -1) {
            ...
            /* SPRD: Bug 602746 add support for MVNO type pnn @{ */
            String[] values = mRes.getStringArray(R.array.mvno_type_entries_ex);
            ...
            if (values[mvnoIndex].equals(NONE)) {
                ...
                /* SPRD: Common feature that apn can't be edited @{ */
                if (mIsEditable) {
            ...
            if (newValue != null && newValue.equals(oldValue) == false) {
                if (values[mvnoIndex].equals("SPN")) {
                    ...
                    /* SPRD: bug 833558 use original spn in sim @{ */
                    /* SPRD: bug 831219 @{ */
                    ...
                    mMvnoMatchData.setSummary(mTelephonyManagerEx.
                            getOriginalServiceProviderName(phoneId));
                    /* @} */
                    /* @} */
            ...
            try {
                /* SPRD: Bug 743019 MVNO type has no translation in ApnEditor @{ */
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_BACK: {
                /* SPRD: add for bug733602 @{ */
                if (!mIsEditable || validateAndSave(false)) {
                    finish();
                }
                /* @} */
                return true;
    ...
    public void onSaveInstanceState(Bundle icicle) {
        ...
        /* SPRD: add for bug733602 @{ */
        if (mIsEditable && validateAndSave(true)) {
            icicle.putInt(SAVED_POS, mCursor.getInt(ID_INDEX));
        }
        /* @} */
        // SPRD: FEATURE_ADD_APN_EDITABLE_FIELD
        icicle.putBoolean(ApnEditorUtils.KEY_IS_EDITABLE, mIsEditable);
    }
    ...
    private boolean validateAndSave(boolean force) {
        ...
        // SPRD: FEATURE_APN_EDITOR_PLUGIN
        if (force && mNewApn && name.length() < 1 && apn.length() < 1 && !mAllowEmptyApn) {
            getContentResolver().delete(mUri, null, null);
            mUri = null;
            return false;
        }
    ...
    private String getErrorMsg() {
        ...
        // SPRD: FEATURE_APN_EDITOR_PLUGIN
        } else if (apn.length() < 1 && !mAllowEmptyApn) {
    ...
    private void deleteApn() {
        /* SPRD: modify for bug712145,720332 @{ */
