...
public class CmccAdvancedWifiSettings extends RestrictedSettingsFragment
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if (KEY_DIALOG_CONNECT_TO_CMCC.equals(key)) {
            ...
            Global.putInt(getContentResolver(), WifiManagerEx.WIFI_CONNECT_ALARM_FLAG, isChecked ? 1 : 0);
            if (isChecked) {
                setConnectWifiAlarm();
            } else {
            ...
            boolean isChecked = ((SwitchPreference) preference).isChecked();
            Global.putInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_FLAG, isChecked ? 1 : 0);
            if (isChecked) {
    ...
    private void initCellularWLANPreference() {
        ...
        hourOfDay = Settings.Global.getInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_HOUR, 0);
        minute = Settings.Global.getInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_MINUTE, 0);
        ...
        switch (dialogId) {
            case DIALOG_WIFI_CONNECT_TIMEPICKER:
                ...
                return new TimePickerDialog(
                        ...
                        Settings.Global.getInt(getContentResolver(),
                                WifiManagerEx.WIFI_CONNECT_ALARM_HOUR, 0),
                        Settings.Global.getInt(getContentResolver(),
                                WifiManagerEx.WIFI_CONNECT_ALARM_MINUTE, 0),
            ...
            case DIALOG_WIFI_DISCONNECT_TIMEPICKER:
                ...
                return new TimePickerDialog(
                        ...
                        //calendar.get(Calendar.MINUTE),
                        Settings.Global.getInt(getContentResolver(),
                                WifiManagerEx.WIFI_DISCONNECT_ALARM_HOUR, 0),
                        Settings.Global.getInt(getContentResolver(),
                                WifiManagerEx.WIFI_DISCONNECT_ALARM_MINUTE, 0),
                        DateFormat.is24HourFormat(getActivity()));
            default:
                break;
        }
    ...
    public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
        ...
        if (whichTimepicker != -1) {
            switch (whichTimepicker) {
                case DIALOG_WIFI_CONNECT_TIMEPICKER:
                    Settings.Global.putInt(getContentResolver(), WifiManagerEx.WIFI_CONNECT_ALARM_HOUR, hourOfDay);
                    Settings.Global.putInt(getContentResolver(), WifiManagerEx.WIFI_CONNECT_ALARM_MINUTE, minute);
                ...
                case DIALOG_WIFI_DISCONNECT_TIMEPICKER:
                    Settings.Global.putInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_HOUR, hourOfDay);
                    Settings.Global.putInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_MINUTE, minute);
    ...
    private void setConnectWifiAlarm() {
        ...
        if (isDismissCalendar(hourOfDay, minute)) {
            inMillis += WifiManagerEx.INTERVAL_MILLIS;
        }
        PendingIntent pendingIntent = PendingIntent.getBroadcast(getActivity(), 0, new Intent(
                WifiManagerEx.ALARM_FOR_CONNECT_WIFI_ACTION), 0);
        mAlarmManager.cancel(pendingIntent);
    ...
    private void setDisonnectWifiAlarm() {
        ...
        int hourOfDay = Settings.Global.getInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_HOUR, 0);
        int minute = Settings.Global.getInt(getContentResolver(), WifiManagerEx.WIFI_DISCONNECT_ALARM_MINUTE, 0);
        Calendar calendar = getCalendar(hourOfDay, minute);
        long inMillis = calendar.getTimeInMillis();
        if (isDismissCalendar(hourOfDay, minute)) {
            inMillis += WifiManagerEx.INTERVAL_MILLIS;
