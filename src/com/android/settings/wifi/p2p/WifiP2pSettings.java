...
public class WifiP2pSettings extends DashboardFragment
    ...
    private static final int DPVALUE = 20;
    private static final int PADDINGTP = 0;
    ...
    private Button mRenamePositiveButton;
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
                ...
                boolean currentGroupFormed = wifip2pinfo.groupFormed;
                if (networkInfo.isConnected()) {
                    if (DBG) Log.d(TAG, "Connected");
                } else if ((mLastGroupFormed != true) || (mLastGroupFormed == true && currentGroupFormed != true)) {
    ...
    protected List<AbstractPreferenceController> getPreferenceControllers(Context context) {
        ...
        PADDING = (int) (DPVALUE * scale + 0.5f);
    ...
    public Dialog onCreateDialog(int id) {
        if (id == DIALOG_DISCONNECT) {
            ...
            AlertDialog dialog = new AlertDialog.Builder(getActivity())
                ...
                .setView(mDeviceNameText,PADDING,PADDINGTP,PADDING,PADDINGTP)
                .setPositiveButton(getActivity().getString(R.string.dlg_ok), mRenameListener)
                .setNegativeButton(getActivity().getString(R.string.dlg_cancel), null)
                //.create();
                .show();
            mRenamePositiveButton = dialog.getButton(AlertDialog.BUTTON_POSITIVE);
            /*SPRD:Add for Bug744238 When the orientation changes, the rename dialog occurs sth wrong @{ */
            try {
                String name = mDeviceNameText.getText().toString().trim();
                if (name.isEmpty()) {
                    mRenamePositiveButton.setEnabled(false);
                } else {
                    mRenamePositiveButton.setEnabled(true);
                }
            } catch (NullPointerException e) {
                mRenamePositiveButton.setEnabled(false);
            }
            /* Bug744238 end @} */
            mDeviceNameText.addTextChangedListener(renameChangedListener);
            return dialog;
    ...
    TextWatcher renameChangedListener = new TextWatcher() {
        @Override
