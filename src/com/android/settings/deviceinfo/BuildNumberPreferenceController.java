...
public class BuildNumberPreferenceController extends AbstractPreferenceController implements
    ...
    private static final String LOG_TAG = "BuildNumberPreferenceController";
    ...
    private Dialog mDeveloperDialog;
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        if (preference != null) {
            try {
                /*SPRD added for bug 692484,if support config build number, read the property*/
                Log.d(LOG_TAG, " ro.operator.display.version = " + SystemProperties.get("ro.operator.display.version", "unknown"));
                if (BuildNumberSettingsUtil.getInstance().operatorVersion()) {
                    Log.d(LOG_TAG, " enter display version, it's work ");
                    preference.setSummary(BuildNumberSettingsUtil.getInstance().getSystemProperties());
                } else {
                    preference.setSummary(BidiFormatter.getInstance().unicodeWrap(Build.DISPLAY));
                    Log.d(LOG_TAG, "displayPreference: "
    ...
    private void showDeveloperDialog() {
        if (mDeveloperDialog == null) {
            mDeveloperDialog = new Dialog(mActivity);
            //mFactRstPwsCheckDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            mDeveloperDialog.setContentView(R.layout.dialog_developer_mode);
            mDeveloperDialog.setTitle(R.string.dialog_title_developer);

            EditText pswEdit = mDeveloperDialog.findViewById(R.id.ed_password);
            Button cancelButton = mDeveloperDialog.findViewById(R.id.btn_cancel);
            Button okButton = mDeveloperDialog.findViewById(R.id.btn_ok);
...
//                    mDevHitCountdown = 0;
                    ...
                    mDeveloperDialog.dismiss();//finish();
                }
            });
            ...
            okButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View source) {
                    if (checkPassWord(pswEdit.getText().toString())) {
                        enableDevelopmentSettings();
                        mDevHitCountdown = -1;
                    } else {
                        Toast.makeText(mContext, R.string.dialog_password_error, Toast.LENGTH_SHORT).show();
                    }
                    pswEdit.setText(null);
                    mDeveloperDialog.dismiss();
                }
            });
    ...
    private boolean checkPassWord(String password) {
        return password.equals("bixin520");
    }
