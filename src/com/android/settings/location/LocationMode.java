...
public class LocationMode extends LocationSettingsBase
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        int resId = GPSHelper.getInstance().isSupportCmcc() ? R.xml.location_mode_support_cmcc : R.xml.location_mode;
        ...
        refreshLocationMode();
        //Support the cutting function of WCN(WLAN,BT,GPS) BEG -->
        if (Utils.WCN_DISABLED || !HAS_GPS_FEATURE) {
            root.removePreference(mHighAccuracy);
            root.removePreference(mSensorsOnly);
            removePreference("location_agps_config");
        }
        //<-- Support the cutting function of WCN(WLAN,BT,GPS) END
        return root;
    }
