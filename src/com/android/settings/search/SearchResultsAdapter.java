...
public class SearchResultsAdapter extends RecyclerView.Adapter<SearchViewHolder>
   ...
   /**
    ...
    public int displaySavedQuery(List<? extends SearchResult> data) {
        clearResults();
        //SPRD: added for bug 862466, data is null, do not add
    ...
    private boolean isLockScreenDisabled() {
        ...
        return lockPatternUtils.isLockScreenDisabled(UserHandle.myUserId());
    }

    public static boolean NotSupportAssist() {
        return SystemProperties.get("ro.product.assistanttouch").equals("false")
    ...
    public void postSearchResults(List<SearchResult> newSearchResults, boolean detectMoves) {
        ...
        mSearchResults.clear();
      //remove lock-settings if nessacery
        String lockSetttings = mContext.getString(R.string.settings_label);
        if (isLockScreenDisabled()) {
            int index = -1;
            for (int i=0; i<newSearchResults.size(); i++) {
                SearchResult sr = newSearchResults.get(i);
                String title = sr.title.toString();
                List<String> breadcrumbs = sr.breadcrumbs;
                if (lockSetttings.equalsIgnoreCase(title) && lockSetttings.equalsIgnoreCase(breadcrumbs.get(0))) {
                    index = i;
                    Log.d(TAG, "lockSetttings:index = " + i);
                    break;
                }
            }
            if (index >= 0) newSearchResults.remove(index);
        }

        //remove assist if nessacery
        String assistant = mContext.getString(R.string.assistant_title);
        if (NotSupportAssist()) {
            int index = -1;
            for (int i=0; i<newSearchResults.size(); i++) {
                SearchResult sr = newSearchResults.get(i);
                String title = sr.title.toString();
                if (assistant.equalsIgnoreCase(title)) {
                    index = i;
                    Log.d(TAG, "assistant:index = " + i);
                    break;
                }
            }
            if (index >= 0) newSearchResults.remove(index);
