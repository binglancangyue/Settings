...
public class AccountPreferenceController extends AbstractPreferenceController
    ...
    public void updateRawDataToIndex(List<SearchIndexableRaw> rawData) {
        ...
        List<UserInfo> profiles = getProfiles(UserHandle.myUserId());
    ...
    private ArrayList<AccountTypePreference> getAccountTypePreferences(AuthenticatorHelper helper,
        ...
        for (int i = 0; i < accountTypes.length; i++) {
            ...
            /*SPRD added for bug 708240, no need to show the Sprd type accounts*/
            ...
            CharSequence label = helper.getLabelForType(mContext, accountType);
            /*SPRD: added for bug 709450*/
            if (accountType.equals(EMAIL_PACKAGE_NAME)){
                label = mContext.getString(R.string.header_category_personal);
            }/**/
            if (label == null) {
                continue;
            }
    ...
    private boolean isSingleProfile() {
        return mUm.isLinkedUser() || getProfiles(UserHandle.myUserId()).size() == 1;
    }

    private List<UserInfo> getProfiles(int userId) {
        List<UserInfo> users = mUm.getProfiles(userId);
        Iterator<UserInfo> iterator = users.iterator();
        while(iterator.hasNext()) {
            UserInfo user = iterator.next();
            if (user != null && AppCloneUserInfo.isAppCloneUserId(user.id)) {
                iterator.remove();
            }
        }
        return users;
