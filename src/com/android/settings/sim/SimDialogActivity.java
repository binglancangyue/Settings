...
public class SimDialogActivity extends Activity {
    ...
    private void setDefaultDataSubIdWithPrompt(final Context context, final int subId) {
        if (subId != SubscriptionManager.getDefaultDataSubscriptionId()) {
            ...
            helper.handleDataSwitch(context,
                    new SettingsOperatorHelper.OnDataSwitchConfirmedCallback(subId) {
                        @Override
                        public void onDataSwitchConfirmed() {
                            Log.d(TAG, "onDataSwitchConfirmed");
                            setDefaultDataSubId(context, subId);
                        }

                        @Override
                        public void onDataSwitchFinished() {
                            Log.d(TAG, "onDataSwitchFinished");
    ...
    private  void setDefaltSubIdByDialogId (int dialogId,List<SubscriptionInfo> subInfoList,int value,final Context context){
        ...
        switch (dialogId) {
            case DATA_PICK:
                ...
                setDefaultDataSubIdWithPrompt(context, sir.getSubscriptionId());
                return;
            case CALLS_PICK:
