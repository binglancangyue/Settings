...
public class SimSelectNotification extends BroadcastReceiver {
    ...
    private static final String SET_PRIMARYCARD_ACTION = "sprd.intent.action.USER_SET_PRIMARYCARD";
    @Override
    public void onReceive(Context context, Intent intent) {
        String action = intent.getAction();
        if(SET_PRIMARYCARD_ACTION.equals(action)){
            Intent setPrimaryCardIntent = new Intent();
            setPrimaryCardIntent.setClass(context, SubSettings.class);
            setPrimaryCardIntent.putExtra(":settings:show_fragment", "com.android.settings.sim.SimSettings");
            setPrimaryCardIntent.putExtra(":settings:show_fragment_as_subsetting", true);
            setPrimaryCardIntent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
            context.startActivity(setPrimaryCardIntent);
            return;
        }
        ...
        // SPRD Modify,remove the clear logic,see bug795334
        ...
        // SPRD: Remove unnecessary notification.
        //createNotification(context);

        // SRPD: Use customized prompt instead of this.
//        if (sil.size() == 1) {
