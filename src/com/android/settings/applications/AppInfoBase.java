...
public abstract class AppInfoBase extends SettingsPreferenceFragment
    ...
    protected String retrieveAppEntry() {
        ...
        /* SPRD: Mofify in order to get the right user of the this application, but not current user {@ */
        int uid = args != null &&
                args.containsKey(AppInfoBase.ARG_PACKAGE_UID) ? args.getInt(AppInfoBase.ARG_PACKAGE_UID) : -1;
        if (uid > 0 && AppCloneUserInfo.isAppCloneUid(uid)) {
        ...
        if (mAppEntry != null) {
            ...
            try {
                /* Modify for appclone to get clone app PackageInfo {@ */
    ...
    public static void startAppInfoFragment(Class<?> fragment, int titleRes,
        ...
        /* SPRD: Modify to start appinfo fragment of clone app in owner settings {@ */
        int userid = UserHandle.getUserId(uid);
        if (AppCloneUserInfo.isAppCloneUid(uid)) {
