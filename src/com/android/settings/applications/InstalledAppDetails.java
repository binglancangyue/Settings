...
/*SPRD: 843838 ADD import for CCSA4 feature @{ */
...
import com.android.settings.applications.defaultapps.DefaultCameraPreferenceController;
import com.android.settings.applications.defaultapps.DefaultEmailPreferenceController;
import com.android.settings.applications.defaultapps.DefaultMusicPreferenceController;
import com.android.settings.applications.defaultapps.DefaultGalleryPreferenceController;
...
public class InstalledAppDetails extends AppInfoBase
    ...
    /* SPRD: 708348 ADD add whilte list for can not kill app @{ */
    private String[] mkeepApps;
    /* @} */

    /* SPRD: Bug 735589 add whilte list for can not click permissions @{ */
    private String[] mPermissionsWhiteList;
    /* @} */
    ...
    boolean handleDisableable(Button button) {
        ...
        /* SPRD: 708348 set the disenable button enabled for white list apps@{ */
        if (mkeepApps != null) {
            for (String pkg : mkeepApps) {
                if (pkg != null && pkg.equals(mAppEntry.info.packageName)) {
                    disableable = false;
                    break;
                }
            }
    ...
    public void onCreate(Bundle icicle) {
        ...
        /* SPRD: 708348,744380 get whilte list for not kill and close permissions app @{ */
        final Resources resources = getResources();
        mkeepApps = resources.getStringArray(R.array.cannot_kill_app);
        mPermissionsWhiteList = resources.getStringArray(R.array.cannot_close_permissions_apps);
        /* @} */
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        /*SPRD: 843838 disable mPermissionsPreference for CCSA4 feature @{ */
        if (SystemProperties.get("persist.support.securetest").equals("1")) {
            getPreferenceScreen().removePreference(mPermissionsPreference);
        }
        /* @} */
        ...
        try {
            String[] canNotSettings = getResources().getStringArray(R.array.cannot_settings_app);
            for (String pkg : canNotSettings) {
                if (pkg != null && pkg.equals(mPackageInfo.packageName)) {
        ...
        /* Add for appclone app, may not support, TODO list {@ */
        if (AppCloneUserInfo.isAppCloneUserId(mUserId)) {
            getPreferenceScreen().removePreference(mPermissionsPreference);
            ...
            getPreferenceScreen().removePreference(mBatteryPreference);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        // SPRD added for bug816793, if package removed by manual,should remove task
        Utils.updatePackageRemovedByMoveStorage(getActivity(), false);
    ...
    void updateBattery() {
        ...
        mBatteryPreference.setEnabled(enable);
            ...
            case DLG_FORCE_STOP:
                return new AlertDialog.Builder(getActivity())
                        ...
                        .setPositiveButton(R.string.dlg_ok, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                /* SPRD:modify for Bug 713141 dialog will show twice in multi-window @{ */
                                if (dialog != null) {
                                    dialog.dismiss();
                                }
                                /* @} */
    ...
    private void forceStopPackage(String pkgName) {
        ...
        //UNISOC, added for bug 937056, NullPointException
    ...
    void checkForceStop() {
        if (mDpm.packageHasActiveAdmins(mPackageInfo.packageName)) {
            ...
            try{
                String[] exceptions = getResources().getStringArray(R.array.cannot_force_stop_app);
    ...
    private void addDynamicPrefs() {
        if (UserManager.get(getContext()).isManagedProfile() || AppCloneUserInfo.isAppCloneUserId(mUserId)) {
            return;
        }
        ...
        /**
         * Add for default app settings
         *@{
         */
        if (DefaultCameraPreferenceController.hasCameraPreference(mPackageName, context)) {
            screen.addPreference(new ShortcutPreference(getPrefContext(),
                    DefaultAppSettings.class, "default_camera_app", R.string.default_camera_title,
                    R.string.configure_apps));
        }
        if (DefaultEmailPreferenceController.hasEmailPreference(mPackageName, context)) {
            screen.addPreference(new ShortcutPreference(getPrefContext(),
                    DefaultAppSettings.class, "default_email_app", R.string.default_email_title,
                    R.string.configure_apps));
        }
        if (DefaultMusicPreferenceController.hasMusicPreference(mPackageName, context)) {
            screen.addPreference(new ShortcutPreference(getPrefContext(),
                    DefaultAppSettings.class, "default_music_app", R.string.default_music_title,
                    R.string.configure_apps));
        }
        if (DefaultGalleryPreferenceController.hasGalleryPreference(mPackageName, context)) {
            screen.addPreference(new ShortcutPreference(getPrefContext(),
                    DefaultAppSettings.class, "default_gallery_app", R.string.default_gallery_title,
                    R.string.configure_apps));
        }
        /*@}*/
        ...
        if ((hasDrawOverOtherApps || hasWriteSettings || hasPictureInPictureActivities ||
                isPotentialAppSource) && !AppCloneUserInfo.isAppCloneUserId(mUserId)) {
    ...
    private void updateDynamicPrefs() {
         ...
         *@{
         */
        pref = findPreference("default_camera_app");
        if (pref != null) {
            pref.setSummary(new DefaultCameraPreferenceController(context)
                    .isCameraDefault(mPackageName)
                    ? R.string.yes : R.string.no);
        }
        pref = findPreference("default_email_app");
        if (pref != null) {
            pref.setSummary(new DefaultEmailPreferenceController(context)
                    .isEmailDefault(mPackageName)
                    ? R.string.yes : R.string.no);
        }
        pref = findPreference("default_music_app");
        if (pref != null) {
            pref.setSummary(new DefaultMusicPreferenceController(context)
                    .isMusicDefault(mPackageName)
                    ? R.string.yes : R.string.no);
        }
        pref = findPreference("default_gallery_app");
        if (pref != null) {
            pref.setSummary(new DefaultGalleryPreferenceController(context)
                    .isGalleryDefault(mPackageName)
                    ? R.string.yes : R.string.no);
        }
        /*@}*/
    ...
    protected void onPackageRemoved() {
        ...
        // SPRD modified for bug816793, if package removed by manual,should remove task
        if (!Utils.isPackageRemovedByMoveStorage(getActivity())) {
        ...
        protected ProcStatsPackageEntry doInBackground(Void... params) {
            ...
            for (ProcStatsPackageEntry pkgEntry : mStatsManager.getEntries()) {
                for (ProcStatsEntry entry : pkgEntry.mEntries) {
                    if (entry.mPackage != null
                            && entry.mPackage.equals (mPackageInfo. applicationInfo.packageName)
                            && entry.mUid == mPackageInfo.applicationInfo.uid) {
    ...
    private final BroadcastReceiver mCheckKillProcessesReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //SPRD 718245 NPE by uninstall apk after received the msg
            if(mAppEntry != null)
    ...
    private final PermissionsResultCallback mPermissionCallback
        ...
        public void onPermissionSummaryResult(int standardGrantedPermissionCount,
            ...
            if (requestedPermissionCount == 0) {
                ...
                /* SPRD: Bug 744380 add whilte list for can not click permissions @{ */
                for (String pkg : mPermissionsWhiteList) {
                    if (pkg != null && mAppEntry != null && mAppEntry.info != null
                            && pkg.equals(mAppEntry.info.packageName)) {
                        mPermissionsPreference.setEnabled(false);
