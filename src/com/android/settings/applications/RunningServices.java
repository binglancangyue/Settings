...
public class RunningServices extends SettingsPreferenceFragment {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        mOptionsMenu = menu;
        menu.add(0, SHOW_RUNNING_SERVICES, 1, R.string.show_running_services).setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        menu.add(0, SHOW_BACKGROUND_PROCESSES, 2, R.string.show_background_processes).setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        /* SPRD: bug 692495 add kill apps menu if it is supportted. @{ */
        if (KillAppsUtils.getInstance().isSupportKillAllApps()) {
            menu.add(0, KILL_APPS_SERVICES, 3, R.string.menu_kill_apps_services).setShowAsAction( MenuItem.SHOW_AS_ACTION_NEVER);
        }/* @} */
        updateOptionsMenu();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case SHOW_BACKGROUND_PROCESSES:
                ...
                break;
            /* SPRD: bug 692495 click the kill all item and then kill all apps. @{ */
