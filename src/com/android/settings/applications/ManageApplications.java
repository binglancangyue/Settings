...
public class ManageApplications extends InstrumentedPreferenceFragment
    ...
    void createHeader() {
        ...
        if (mListType == LIST_TYPE_MAIN) {
            /* Modify for appclone, ignore app clone user info {@ } */
            List<UserHandle> profiles = UserManager.get(getActivity()).getUserProfiles();
            Iterator<UserHandle> it = profiles.iterator();
            while (it.hasNext()) {
                UserHandle userHandle = it.next();
                if (AppCloneUserInfo.isAppCloneUserId(userHandle.getIdentifier())) {
                    it.remove();
                }
            }
            if (profiles.size() > 1) {
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        private void updateSummary(AppViewHolder holder) {
            switch (mManageApplications.mListType) {
                case LIST_TYPE_NOTIFICATION:
                    /* SPRD Bug: 796382 monkey test, and happens ClassCastException */
