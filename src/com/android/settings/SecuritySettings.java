...
import com.android.settings.faceid.FaceIdEnrollIntroduction;
import com.android.settings.faceid.FaceIdSettings;
...
public class SecuritySettings extends SettingsPreferenceFragment
    ...
    //add for facelock feature
    private static final String KEY_FACEID_SETTINGS_SWITCH = "faceid_settings_switch";
    private static final String KEY_FACEID_SETTINGS = "faceid_settings";
    private static final String FACEID_SETTINGS_SWITCH = "faceid_settings_switch";
    ...
    //add for facelock feature
    private Preference mFaceIdPreference;
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        if (securityCategory != null) {
            ...
            if (faceManager != null && faceManager.isHardwareDetected()) {
                maybeAddFaceIdPreference(securityCategory);
            } else {
                Log.v(TAG, "No face lock hardware detected!!");
            }
        }
    ...
    //add for facelock feature
    private void maybeAddFaceIdPreference(PreferenceGroup securityCategory) {
        FaceManager faceManager = (FaceManager) getSystemService(Context.FACE_SERVICE);
        if (!faceManager.isHardwareDetected()) {
            Log.v(TAG, "No faceid hardware detected!!");
            return;
        }
        mFaceIdPreference = new Preference(securityCategory.getContext());
        mFaceIdPreference.setKey(KEY_FACEID_SETTINGS);
        mFaceIdPreference.setTitle(R.string.security_settings_faceid_preference_title);
        Intent intent = new Intent();
        final List<Face> items = faceManager.getEnrolledFaces();
        final int faceidCount = items != null ? items.size() : 0;
        final String clazz;
        if (faceidCount >= 1) {
            mFaceIdPreference.setSummary(R.string.security_settings_faceid_preference_summary);
            clazz = FaceIdSettings.class.getName();
        } else {
            clazz = FaceIdEnrollIntroduction.class.getName();
        }
        intent.setClassName("com.android.settings", clazz);
        mFaceIdPreference.setIntent(intent);
        securityCategory.addPreference(mFaceIdPreference);
    }
    ...
    public static class SecuritySubSettings extends SettingsPreferenceFragment
        ...
        private void createPreferenceHierarchy() {
            ...
            //SPRD 750005 can not find res by incorect resid,which caused crash
        ...
        private void updateLockAfterPreferenceSummary() {
            ...
            if (mLockAfter.isDisabledByAdmin()) {
                ...
                if (!TextUtils.isEmpty(trustAgentLabel)) {
                    ...
                    /* SPRD:Bug 713450 modify to make the string correct if user choose first item "immediately"*/
                     if (best == 0) {
