    ...
    private PackageInfo findPackageInfo(String pkg, int uid) {
        ...
        if (packages != null && pkg != null) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (pkg.equals(p)) {
                    try {
                        // SPRD: Modify in order to get packageinfo of clone app, but not app of current user
                        return mPm.getPackageInfoAsUser(pkg, PackageManager.GET_SIGNATURES, UserHandle.getUserId(uid));
                    } catch (NameNotFoundException e) {
